...
# =============================================================================
# PART 2: PICOCHAT ABLATIONS
# Add this block anywhere after the helper functions (_python, _torchrun, etc.)
# and before the quick_test function.
# =============================================================================
...
@app.function(
    ...
    gpu=PICO_GPUS,
    timeout=60 * 60,  # 1 hour max (should take ~10 min)
)
def stage_picochat(run_name: str = "picochat_baseline") -> None:
    """
    Train a picochat (depth=8, ~40M params) for Part 2 ablations.

    Run this 4 times with different run_name values:
        picochat_baseline  -- no architecture changes
        picochat_ptb       -- Parallel Transformer Block only
        picochat_rope      -- Higher RoPE base theta (100K) only
        picochat_both      -- PTB + RoPE together

    Make sure to edit nanochat/gpt.py between runs to apply the changes.
    Modal re-uploads your local code every time via .add_local_dir so
    each run picks up your latest gpt.py automatically.

    Usage:
        modal run nanochat_modal.py::stage_picochat --run-name picochat_baseline
        # edit gpt.py for PTB
        modal run nanochat_modal.py::stage_picochat --run-name picochat_ptb
        # revert PTB, add RoPE base=100000
        modal run nanochat_modal.py::stage_picochat --run-name picochat_rope
        # add PTB back too
        modal run nanochat_modal.py::stage_picochat --run-name picochat_both
    """
    _setup_cache()

    # Make sure we have enough shards downloaded
    print(f"Ensuring {PICO_SHARDS} shards are available...")
    _python("nanochat.dataset", [f"-n {PICO_SHARDS}"])
    volume.commit()

    print(f"Starting picochat training: run={run_name}, depth={PICO_DEPTH}")
    _torchrun(
        "scripts.base_train",
        [
            f"--depth={PICO_DEPTH}",
            ...
            f"--run={run_name}",
            "--window-pattern=SSSL",
            "--save-every=-1",  # no intermediate checkpoints (run is short)
            "--sample-every=-1",  # no intermediate samples
            # Keep CORE eval at the end (it runs automatically at final step)
            # Remove the line below if you want to skip it and save ~5 min
    ...
    volume.commit()
    print(f"Picochat run complete: {run_name}")
...
def quick_test() -> None:
    ...
    identity_dest = os.path.join(NANOCHAT_CACHE, "identity_conversations.jsonl")
    _curl(IDENTITY_JSONL_URL, identity_dest)
    _torchrun("scripts.chat_sft", [f"--run={WANDB_RUN}"], nproc=nproc)
